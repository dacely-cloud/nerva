...
use crate::weights::{
    ...
    ResidentWeightPrefetchPlan, ResidentWeightPrefetchTask, ResidentWeightProbeStatus,
    ResidentWeightProbeSummary, ResidentWeightTable,
};
#[cfg(test)]
...
fn update_prefetch_data_hash(mut hash: u64, bytes: &[u8]) -> u64 {
    for byte in bytes {
        hash ^= u64::from(*byte);
