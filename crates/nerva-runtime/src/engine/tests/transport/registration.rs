...
fn transport_registration_probe_reuses_stable_block_registrations() {
    ...
    let direct_verified =
        summary.gpu_direct_rdma_capability == CapabilityState::SupportedAndVerified;

    assert_eq!(summary.status, TransportRegistrationStatus::Ok);
    assert_eq!(
        summary.bootstrap_registrations,
        summary.pinned_host_registrations + summary.gpu_direct_registrations
    );
    assert_eq!(summary.registered_entries, summary.bootstrap_registrations);
    assert_eq!(summary.cache_hits, summary.bootstrap_registrations);
    ...
    assert_eq!(summary.false_gpu_direct_registrations, 0);
    assert_eq!(
        summary.gpu_direct_registrations,
        if direct_verified { 1 } else { 0 }
    );
    assert_eq!(
        summary.gpu_direct_registration_skips,
        if direct_verified { 0 } else { 1 }
    );
    assert_eq!(summary.hot_path_allocations, 0);
    assert_eq!(summary.transport_events, summary.cache_hits);
    ...
    assert!(json.contains("\"gpu_direct_rdma_capability\""));
    assert!(json.contains("\"gpu_direct_registration_skips\""));
    assert!(json.contains("\"false_gpu_direct_registrations\":0"));
