...
use crate::engine::compute_near_data::decisions::{
    ComputeNearDataCosts, record_cpu_shard_decision, record_gpu_shard_decision,
};
...
pub(crate) fn execute_resident_split_matvec(
    ...
    for shard in shards {
        execute_shard(registry, device, cols, input, shard, costs, output, ledger)?;
    }
...
fn execute_shard(
    ...
    match shard.tier {
        MemoryTier::Dram => execute_dram_shard(cols, input, shard, costs, output, ledger),
        MemoryTier::Vram => execute_vram_shard(device, cols, input, shard, costs, output, ledger),
...
fn execute_dram_shard(
    ...
    record_cpu_shard_decision(shard, costs, ledger);
    mat_vec_row_range(
        shard.weights,
        input,
        cols,
    ...
    ledger.record(LedgerEvent {
        ...
        latency_ns: costs.cpu_kernel_ns,
...
fn execute_vram_shard(
    ...
    record_gpu_shard_decision(device, shard, costs, ledger);
    mat_vec_row_range(
        shard.weights,
        input,
        cols,
