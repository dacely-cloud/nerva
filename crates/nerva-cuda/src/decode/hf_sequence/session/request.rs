...
use crate::decode::hf_sequence::session::ffi::{
    ...
    snapshot_deepseek_v3_mla_kv, snapshot_deepseek_v4_compressed_kv,
    snapshot_deepseek_v4_indexer_kv, snapshot_deepseek_v4_mhc, snapshot_deepseek_v4_swa_kv,
...
pub type CudaHfDecodeSequenceDeepSeekV4IndexerKvSnapshot =
    CudaHfDecodeSequenceDeepSeekV4SwaKvSnapshot;
...
impl CudaHfDecodeSequenceSession {
    ...
    pub fn deepseek_v32_mla_packed_kv_snapshot(
        &mut self,
        layer_index: u32,
        byte_capacity: usize,
    ) -> CudaHfDecodeSequenceDeepSeekV3MlaKvSnapshot {
        if byte_capacity == 0 {
            return CudaHfDecodeSequenceDeepSeekV3MlaKvSnapshot {
                ...
                bytes: Vec::new(),
                error: Some(
                    "DeepSeek V3.2 packed MLA KV snapshot requires a byte capacity".to_string(),
                ),
            };
        }
        let mut bytes = vec![0u8; byte_capacity];
        ...
        let mut out = NervaCudaHfDecodeSequenceDeepSeekV3MlaKvSnapshotResult::default();
        let return_code = snapshot_deepseek_v32_mla_packed_kv(&request, &mut out);
        let copied = out.copied_bytes.min(byte_capacity as u64) as usize;
        bytes.truncate(copied);
        let status = if return_code == 0 && out.status == 0 {
            SmokeStatus::Ok
        ...
        let error = (status != SmokeStatus::Ok).then(|| {
            format!(
                "DeepSeek V3.2 packed MLA KV snapshot failed: return_code={return_code} status={} cuda_error={}",
                out.status, out.cuda_error
            )
        });
        CudaHfDecodeSequenceDeepSeekV3MlaKvSnapshot {
    ...
    pub fn deepseek_v32_indexer_kv_snapshot(
        &mut self,
        layer_index: u32,
        byte_capacity: usize,
    ) -> CudaHfDecodeSequenceDeepSeekV3MlaKvSnapshot {
        if byte_capacity == 0 {
            return CudaHfDecodeSequenceDeepSeekV3MlaKvSnapshot {
                ...
                bytes: Vec::new(),
                error: Some(
                    "DeepSeek V3.2 indexer KV snapshot requires a byte capacity".to_string(),
                ),
            };
        }
        let mut bytes = vec![0u8; byte_capacity];
        ...
        let mut out = NervaCudaHfDecodeSequenceDeepSeekV3MlaKvSnapshotResult::default();
        let return_code = snapshot_deepseek_v32_indexer_kv(&request, &mut out);
        let copied = out.copied_bytes.min(byte_capacity as u64) as usize;
        bytes.truncate(copied);
        let status = if return_code == 0 && out.status == 0 {
            SmokeStatus::Ok
    ...
    pub fn deepseek_v4_indexer_kv_snapshot(
        &mut self,
        layer_index: u32,
        byte_capacity: usize,
    ) -> CudaHfDecodeSequenceDeepSeekV4IndexerKvSnapshot {
        if byte_capacity == 0 {
            return CudaHfDecodeSequenceDeepSeekV4IndexerKvSnapshot {
                status: SmokeStatus::Failed,
                cuda_error: 0,
                layer_index,
                block_count: 0,
                ...
                output_hash: 0,
                bytes: Vec::new(),
                error: Some("DeepSeek V4 indexer KV snapshot requires a byte capacity".to_string()),
            };
        }
        let mut bytes = vec![0u8; byte_capacity];
        let request = NervaCudaHfDecodeSequenceDeepSeekV4CompressedKvSnapshotRequest {
        ...
        let mut out = NervaCudaHfDecodeSequenceDeepSeekV4CompressedKvSnapshotResult::default();
        let return_code = snapshot_deepseek_v4_indexer_kv(&request, &mut out);
        let copied = out.copied_bytes.min(byte_capacity as u64) as usize;
        bytes.truncate(copied);
        let status = if return_code == 0 && out.status == 0 {
            SmokeStatus::Ok
        ...
        let error = (status != SmokeStatus::Ok).then(|| {
            format!(
                "DeepSeek V4 indexer KV snapshot failed: return_code={return_code} status={} cuda_error={}",
                out.status, out.cuda_error
            )
        });
        CudaHfDecodeSequenceDeepSeekV4IndexerKvSnapshot {
