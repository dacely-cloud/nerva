...
#[test]
fn hf_decode_sequence_session_verifies_greedy_draft_block() {
    let _guard = super::cuda_test_lock();
    ...
    assert_eq!(started.summary.status, SmokeStatus::Ok);
    let mut loop_state = started.loop_state.unwrap();
    let summary = loop_state.verify_block(&[0, 0, 0]);

    assert_eq!(summary.status, SmokeStatus::Ok);
    assert_eq!(summary.tokens, vec![0, 0, 0]);
    assert_eq!(summary.h2d_bytes, 8);
    assert_eq!(summary.host_causality_edges, 0);
    assert_eq!(summary.hot_path_allocations, 0);
    assert!(summary.kernel_launches > 0);
    assert!(summary.device_elapsed_ns > 0);
}
