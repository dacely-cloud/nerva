...
fn deepseek_v4_runtime_plan_reports_vllm_gap_and_layer_mix() {
    ...
    assert!(json.contains("deepseek_mla_prefill_causal_mqa_reference"));
    ...
    assert!(json.contains("deepseek_v3_full_routed_moe_noaux_tc_reference"));
    assert!(json.contains("cuda_deepseek_routed_moe_api"));
    assert!(json.contains("deepseek_full_routed_moe_reference"));
    ...
    assert!(json.contains("deepseek_qkv_rmsnorm_reference"));
    ...
    assert!(json.contains("deepseek_c128_topk_metadata_reference"));
    ...
    assert!(json.contains("cuda_hf_sequence_deepseek_v4_c128_parallel_head_attention_runtime"));
...
fn deepseek_v32_runtime_plan_reports_sparse_indexer_requirement() {
    ...
    assert!(!json.contains("fuse block-FP8 dequant with projection GEMM"));
    assert!(json.contains(
...
fn deepseek_cuda_readiness_reports_smokes_and_runtime_gaps() {
    ...
    assert!(json.contains("cuda_hf_sequence_deepseek_v4_sparse_attention_swa_plus_topk"));
    assert!(json.contains("cuda_hf_sequence_deepseek_v4_c4_topk_cover_all_shortcut"));
    assert!(json.contains("run full-size V4 C4 sparse top-k differential against vLLM runtime"));
