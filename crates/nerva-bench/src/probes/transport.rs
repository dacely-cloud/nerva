...
pub(crate) fn run_transport_metric_provenance_probe() -> Result<String, String> {
    let runtime = Runtime::new(RuntimeConfig::default())
        .map_err(|err| format!("runtime init failed: {err:?}"))?;
    let summary = runtime
        .run_transport_metric_provenance_probe()
        .map_err(|err| format!("transport metric provenance probe failed: {err:?}"))?;
    Ok(summary.to_json())
}
...
pub(crate) fn run_transport_registration_lifecycle_probe() -> Result<String, String> {
    let runtime = Runtime::new(RuntimeConfig::default())
        .map_err(|err| format!("runtime init failed: {err:?}"))?;
    let summary = runtime
        .run_transport_registration_lifecycle_probe()
        .map_err(|err| format!("transport registration lifecycle probe failed: {err:?}"))?;
    Ok(summary.to_json())
}
