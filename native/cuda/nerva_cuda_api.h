...
typedef struct NervaCudaHfDecodeSequenceResult {
  ...
  uint64_t deepseek_sparse_attention_output_hash;
...
typedef struct NervaCudaDeepSeekQuantFp8F32ScaleEncodedGemmTokensRequest {
  uint32_t rows;
  uint32_t cols;
  uint32_t tokens;
  uint32_t block_rows;
...
typedef struct NervaCudaDeepSeekMegaMoeExpertsRequest {
  uint32_t num_tokens;
  uint32_t hidden_size;
  uint32_t intermediate_size;
  uint32_t num_experts;
  ...
  float *output;
} NervaCudaDeepSeekMegaMoeExpertsRequest;

typedef struct NervaCudaDeepSeekMegaMoeExpertsResult {
  int32_t status;
  int32_t cuda_error;
  int32_t device_count;
  ...
  uint64_t sync_calls;
  uint64_t hot_path_allocations;
} NervaCudaDeepSeekMegaMoeExpertsResult;
...
int nerva_cuda_hf_decode_sequence_deepseek_v32_indexer_kv_snapshot(
    const NervaCudaHfDecodeSequenceDeepSeekV3MlaKvSnapshotRequest *request,
    NervaCudaHfDecodeSequenceDeepSeekV3MlaKvSnapshotResult *out);
int nerva_cuda_hf_decode_sequence_deepseek_v32_indexer_query_state_snapshot(
    const NervaCudaHfDecodeSequenceDeepSeekV3MlaKvSnapshotRequest *request,
    NervaCudaHfDecodeSequenceDeepSeekV3MlaKvSnapshotResult *out);
...
int nerva_cuda_deepseek_megamoe_experts(
